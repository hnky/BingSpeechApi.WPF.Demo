...
namespace BingSpeechDemo.TextToSpeech
{
    ...
    public class Authentication
    {
        ...
        private void OnTokenExpiredCallback(object stateInfo)
        {
            ...
            finally
            {
                ...
                {
                    Console.WriteLine(
                        string.Format("Failed to reschedule the timer to renew access token. Details: {0}",
                            ex.Message));
                }
            }
        }
    ...
    public class Synthesize
    {
        ...
        public Task Speak(CancellationToken cancellationToken, InputOptions inputOptions)
        {
            ...
            {
                Content = new StringContent(GenerateSsml(inputOptions.Locale, genderValue, inputOptions.VoiceName,
                    inputOptions.Text))
            };
            ...
            var saveTask = httpTask.ContinueWith(
                async (responseMessage, token) =>
                {
                    try
                    {
                        if (responseMessage.IsCompleted && responseMessage.Result != null &&
                            responseMessage.Result.IsSuccessStatusCode)
                        {
                            var httpStream = await responseMessage.Result.Content.ReadAsStreamAsync()
                                .ConfigureAwait(false);
                            this.AudioAvailable(new GenericEventArgs<Stream>(httpStream));
                        }
                        else
                        {
                            this.Error(new GenericEventArgs<Exception>(
                                new Exception(String.Format("Service returned {0}",
        ...
        public class InputOptions
        {
            ...
            public IEnumerable<KeyValuePair<string, string>> Headers
            {
                get
                {
                    ...
                    // Refer to the doc
                    toReturn.Add(
                        new KeyValuePair<string, string>("X-Search-AppId", "07D3234E49CE426DAA29772419F436CA"));
                    // Refer to the doc
                    toReturn.Add(new KeyValuePair<string, string>("X-Search-ClientID",
